...
module uart #(
  ...
  localparam bit [RegAddr-1:0] UartBaudDiv = RegAddr'('hC); //Einfach Clk/Baudrate davor berechnet
  localparam int unsigned ClocksPerBaud      = ClockFrequency / BaudRate; //Davor ClockFrequency / Baudrate
  ...
  logic [15:0] rx_baud_counter_q, rx_baud_counter_d;
  logic                             rx_baud_tick;
  ...
  logic [15:0] tx_baud_counter_q, tx_baud_counter_d;
  logic                             tx_baud_tick;
  ...
  logic       tx_fifo_full;
  logic [15:0] bauddivreg;
  assign reg_addr = device_addr_i[RegAddr-1:0];
  logic set_baud;
  //Wenn reg_addr genau UartBaudDiv entspricht (0x0000000C) und es einen write request
  //gibt, dann wird set_baud auf high gesetzt und device_wdata_i wird in bauddivreg geschrieben
  assign set_baud = (reg_addr == UartBaudDiv) & device_req_i & device_be_i[0] & device_we_i;
  //Baudrate setzen können:
  always_ff @(posedge clk_i or negedge rst_ni) begin
    if (!rst_ni) begin
       bauddivreg <= ClocksPerBaud; //hardkodierter standart wert bei reset
    end else begin
      if(set_baud)begin
        bauddivreg <= device_wdata_i[15:0];
      end
    end
  end
//Hier wird aus registern gelesen:
  always_comb begin
    device_rdata_d  = '0;
    ...
    if (device_req_i) begin
      ...
      if (device_be_i[0] & ~device_we_i) begin
        case (reg_addr)
          ...
          UartBaudDiv: begin
            //TODO: Baudrate register soll auch gelesen werden können
            device_rdata_d = bauddivreg;
          end
          default: begin
            device_rdata_d = '0;
          end
  ...
  assign rx_baud_counter_d = rx_baud_tick ? '0                                            :
                             rx_start     ? $bits(rx_baud_counter_q)'(bauddivreg >> 1) :
                                            rx_baud_counter_q + 1'b1;

  assign rx_baud_tick = rx_baud_counter_q == $bits(rx_baud_counter_q)'(bauddivreg - 1);
  ...
  assign tx_baud_tick      = tx_baud_counter_q == $bits(tx_baud_counter_q)'(bauddivreg - 1);
